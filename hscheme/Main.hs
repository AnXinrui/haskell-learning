...
import qualified SchemeParser as SP
...
evalDefinition var exp envRef = do
    value <- eval exp envRef
    env <- readIORef envRef
    let newFrame = (var, value):(case env of [] -> []; (f:_) -> f)
    writeIORef envRef (newFrame:(case env of [] -> []; (_:fs) -> fs))
...
lookupBinding name [] = error $ "Undefined variable: " ++ name
lookupBinding name env = case M.mapMaybe (lookup name) env of
...
buildExpression env (SP.SchemeList ((SP.SchemeAtom op):params)) = 
    case op of
      "if" -> If cond consq alt 
          where (cond:consq:alt:[]) = map (buildExpression env) params
      "begin" -> Begin $ map (buildExpression env) params
      "define" -> case params of
          [SP.SchemeList ((SP.SchemeAtom var):args), body] -> 
              Definition var (Lambda (map extractVarName args) [buildExpression env body])
          [SP.SchemeAtom var, val] -> 
              Definition var (buildExpression env val)
          _ -> error "Invalid define syntax"
...
-- Helper function to extract variable names from SchemeVal
extractVarName :: SP.SchemeVal -> VarName
extractVarName (SP.SchemeAtom var) = var
extractVarName _ = error "Expected variable name"
